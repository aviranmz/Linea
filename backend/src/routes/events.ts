...
export async function eventRoutes(fastify: FastifyInstance, options: FastifyPluginOptions) {
  // Get all events
  fastify.get('/', {
    schema: {
    ...
    const { page, limit, status, category, search } = request.query as z.infer<typeof eventRoutes.querystring>
    
    // TODO: Implement event listing with filters and pagination
    fastify.log.info(`Fetching events: page=${page}, limit=${limit}, status=${status}`)
