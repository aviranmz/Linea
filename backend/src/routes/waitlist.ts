...
export async function waitlistRoutes(
  ...
  fastify.post(
    ...
    async (request, reply) => {
      const { email, eventId } = request.body as z.infer<
  ...
  fastify.get(
    ...
    async (request, reply) => {
      const { eventId } = request.params as z.infer<
        typeof waitlistRoutes.params
      >;
      const { page, limit, status } = request.query as z.infer<
        typeof waitlistRoutes.querystring
      >;
