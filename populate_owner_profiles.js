...
const ownerProfiles = [
  {
    ...
    businessIntro: 'Award-winning interior design studio specializing in contemporary Italian design. We create beautiful, functional spaces that reflect our clients\' personalities and lifestyles.',
  ...
  },
  {
    ...
    businessIntro: 'Sustainable architecture firm focused on innovative design solutions. We believe in creating spaces that harmonize with nature while meeting modern living needs.',
    ...
    logoUrl: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=200&h=200&fit=crop&crop=center',
    profilePictureUrl: 'https://images.unsplash.com/photo-1494790108755-2616b612b786?w=200&h=200&fit=crop&crop=face'
  }
]
...
async function populateOwnerProfiles() {
  ...
  try {
    ...
    if (areas.length === 0 || products.length === 0) {
      console.log('❌ No areas or products found. Please run the seed script first.')
      return
    }
    ...
    for (const profileData of ownerProfiles) {
      ...
      // Add photo gallery
      const photoGallery = photoGalleries.find(pg => pg.email === profileData.email)
      if (photoGallery) {
        ...
        console.log(`✅ Added ${photoGallery.photos.length} photos for ${profileData.email}`)
      }
    }
