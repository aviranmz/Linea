...
export function EventCard({
  ...
  const getEventLocation = () => {
    ...
    return fallbacks.venue
      ? `${fallbacks.venue.city}, ${fallbacks.venue.country}`
      : 'Location TBA';
  };
      ...
      >
        <div className='flex items-start space-x-3'>
          ...
          <div className='flex-1 min-w-0'>
            ...
            <p className='text-xs text-gray-500'>{getEventLocation()}</p>
            {showWaitlist && getWaitlistText() && (
              <p className='text-xs text-blue-600 mt-1'>{getWaitlistText()}</p>
            )}
          </div>
        </div>
      </div>
  ...
  if (variant === 'featured') {
    return (
      <div
        className={`bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-200 p-6 hover:shadow-lg transition-shadow ${className}`}
      >
          ...
          <div className='flex items-center space-x-2'>
            {displayData.category && (
              <span
                ...
                style={{
                  backgroundColor: displayData.category.color + '20',
                  color: displayData.category.color,
                }}
              >
        ...
        <p className='text-gray-600 mb-4'>{getEventDescription()}</p>
        ...
        <div className='flex items-center justify-between'>
          ...
          {showWaitlist && getWaitlistText() && (
            <span className='text-sm text-blue-600'>{getWaitlistText()}</span>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div
      className={`bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow ${className}`}
    >
      ...
      <div className='p-4'>
        ...
        <p className='text-gray-600 text-sm mb-3'>{getEventDescription()}</p>
        ...
        {getCapacityText() && (
          <div className='mt-2 text-sm text-gray-500'>{getCapacityText()}</div>
        )}
      </div>
    </div>
  );
