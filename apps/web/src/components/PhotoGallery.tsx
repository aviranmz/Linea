...
export function PhotoGallery({ ownerId, isEditable = false, maxPhotos = 12 }: PhotoGalleryProps) {
  ...
  const loadPhotos = async () => {
    try {
      const data = await getJson<{ photos: Photo[] }>('/api/owner/photo-gallery')
  ...
  const handleAddPhoto = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      const photo = await postJson<{ photo: Photo }>('/api/owner/photo-gallery', formData)
      setPhotos(prev => [...prev, photo.photo])
      setShowAddModal(false)
      setFormData({ title: '', description: '', imageUrl: '', thumbnailUrl: '', altText: '', order: 0 })
    } catch (err) {
  ...
  const handleEditPhoto = async (e: React.FormEvent) => {
    ...
    try {
      const photo = await putJson<{ photo: Photo }>(`/api/owner/photo-gallery/${editingPhoto.id}`, formData)
      setPhotos(prev => prev.map(p => p.id === photo.photo.id ? photo.photo : p))
