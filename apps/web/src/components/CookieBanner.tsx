      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-6">
        <div className="bg-whitebg-neutral-900 border border-neutral-200border-neutral-700 rounded-2xl shadow-milano p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="text-sm text-neutral-700text-neutral-200">
            {t('cookie.message')} <a className="underline hover:text-accent-600" href="/privacy">{t('cookie.privacyPolicy')}</a>.
          </div>
          <div className="ml-auto flex gap-2">
            <button onClick={decline} className="btn btn-outline">{t('cookie.decline')}</button>
            <button onClick={acceptAll} className="btn btn-primary">{t('cookie.acceptAll')}</button>
          </div>
        </div>
      </div>
    </div>
