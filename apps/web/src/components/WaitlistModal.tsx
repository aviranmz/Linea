...
export function WaitlistModal({ event, isOpen, onClose }: WaitlistModalProps) {
  ...
  const updateStatus = async (entryId: string, newStatus: 'PENDING' | 'CONFIRMED' | 'CANCELLED') => {
    try {
      await postJson(`/api/owner/events/${event?.id}/waitlist/${entryId}`, { status: newStatus })
      ...
      <div className="flex min-h-screen items-start justify-center p-4 pt-20">
        ...
        <div className="relative w-full max-w-6xl bg-white rounded-lg shadow-xl z-[10000] max-h-[calc(100vh-8rem)] flex flex-col">
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-white rounded-t-lg flex-shrink-0">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Waitlist: {event.title}</h2>
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            {loading ? (
              <div className="flex items-center justify-center py-12">
                ...
                <span className="ml-3 text-gray-600">{t('api.loadingWaitlist')}</span>
              </div>
            ) : waitlistEntries.length === 0 ? (
              <div className="text-center py-12">
                <div className="text-gray-400 text-6xl mb-4">📝</div>
                <h3 className="text-lg font-medium text-gray-900 mb-2">{t('api.noWaitlistEntries')}</h3>
                <p className="text-gray-600">{t('api.noWaitlistEntriesDescription')}</p>
              </div>
            ) : (
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div>
                            ...
                            {entry.user?.businessName && (
                              <div className="text-xs text-gray-400">{entry.user.businessName}</div>
                            )}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
