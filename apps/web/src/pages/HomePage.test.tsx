...
describe('HomePage', () => {
  ...
  it.skip('renders hero section', () => {
    ...
    expect(
      screen.getByText('Discover Extraordinary Events')
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        'Join exclusive events through our innovative waitlist platform. Experience the future of event management with email-only access.'
      )
    ).toBeInTheDocument();
  });
  ...
  it.skip('shows empty state when no events', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('Check back soon for exciting events!')
      ).toBeInTheDocument();
    });
  });
  ...
  it.skip('renders CTA section', () => {
    ...
    expect(
      screen.getByText('Start building your community today.')
    ).toBeInTheDocument();
  });
});
