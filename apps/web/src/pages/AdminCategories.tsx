...
export default function AdminCategories() {
  ...
  const loadCategories = async () => {
    try {
      const data = await getJson<{ categories: Category[] }>('/api/admin/categories')
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewCategory({ name: '', slug: '', description: '', color: '#3b82f6', icon: '🏷️', isActive: true })
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await putJson(`/api/admin/categories/${editingCategory.id}`, editingCategory)
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600">Manage event categories and tags</p>
        </div>
        <button
