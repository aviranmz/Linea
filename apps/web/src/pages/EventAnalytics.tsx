...
export default function EventAnalytics() {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      const data = await getJson<EventAnalyticsData>(`/api/analytics/event/${eventId}?period=${period}`)
      setAnalytics(data)
      
      // Also fetch event title for display
      try {
        const eventData = await getJson<{ event: { title: string } }>(`/api/owner/events/${eventId}`)
      ...
      if (error.status === 404) {
        setError(`Event not found. The event with ID "${eventId}" does not exist or may have been deleted.`)
      } else if (error.status === 403) {
        setError('Access denied. You do not have permission to view analytics for this event.')
        // Redirect to owner portal after 3 seconds
        setTimeout(() => navigate('/owner'), 3000)
      } else {
        setError(`Failed to load analytics data. Error: ${error.message || 'Unknown error'}`)
      }
    } finally {
      setLoading(false)
    }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        <div className="card p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-neutral-900">{analytics.metrics.totalViews.toLocaleString()}</p>
            </div>
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-neutral-900">{analytics.metrics.uniqueUsers.toLocaleString()}</p>
            </div>
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-neutral-600">Interactions</p>
              <p className="text-3xl font-bold text-neutral-900">{analytics.metrics.totalInteractions.toLocaleString()}</p>
            </div>
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-neutral-900">
                {analytics.metrics.totalViews > 0 
                  ? ((analytics.metrics.totalInteractions / analytics.metrics.totalViews) * 100).toFixed(1)
                  : '0'
                }%
              </p>
            </div>
                ...
                <div key={country.country} className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="w-6 h-6 bg-accent-100 rounded-full flex items-center justify-center">
                      <span className="text-xs font-semibold text-accent-600">#{index + 1}</span>
                    </div>
      ...
      {analytics.metrics.recentViews.length > 0 && (
        <div className="card p-6">
          <h3 className="text-lg font-semibold text-neutral-900 mb-4">Recent Views</h3>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-neutral-200">
                  ...
                  <th className="text-left py-3 px-4 text-sm font-medium text-neutral-600">Device</th>
                    ...
                    <td className="py-3 px-4">
                      <div>
                        ...
                        {view.userEmail && view.userName && (
                          <p className="text-xs text-neutral-500">{view.userEmail}</p>
                        )}
                      </div>
                    </td>
