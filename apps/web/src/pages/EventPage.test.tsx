...
describe('EventPage', () => {
  ...
  it('displays event details when loaded', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/event\.capacity\s*:\s*100/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it('shows not found when event does not exist', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("The event you're looking for doesn't exist.")
      ).toBeInTheDocument();
    });
  });
  ...
  it('validates email input', async () => {
    ...
    expect(
      screen.queryByText('Successfully joined the waitlist!')
