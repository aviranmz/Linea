...
export default function AdminOwners() {
  ...
  const exportCsv = () => {
    const header = ['id','email','name','phone','businessName','website','address','city','country','status','createdAt','eventCount']
    const rows = sortedOwners.map(o => [
      o.id, o.email, o.name||'', o.phone||'', o.businessName||'', o.website||'', 
      o.address||'', o.city||'', o.country||'', o.status, o.createdAt, String(o.eventCount)
    ])
  ...
  const toggleOwnerStatus = async (owner: OwnerRow) => {
    try {
      ...
      await putJson(`/api/admin/owners/${owner.id}/status`, { status: newStatus })
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex items-end justify-between mb-6 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-600">Manage owner accounts and details</p>
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              ...
              <tr><td className="px-4 py-6" colSpan={5}>No owners found.</td></tr>
            ) : sortedOwners.map(o => (
              <tr key={o.id} className="hover:bg-gray-50">
                <td className="px-4 py-2">
                  <div className="text-sm font-medium text-gray-900">{o.name || '—'}</div>
                  <div className="text-xs text-gray-500">{o.email}</div>
                  {o.businessName && <div className="text-xs text-gray-400">{o.businessName}</div>}
                </td>
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">{Math.min(page * limit, owners.length)}</span> of{' '}
