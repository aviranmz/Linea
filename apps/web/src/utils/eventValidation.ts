...
export const validateEventForm = (
  ...
  if (!formData.title) {
    ...
    errors.title = [
      t
        ? t('validation.titleMinLength', {
            min: EVENT_VALIDATION_RULES.title.minLength,
          })
    ...
    errors.title = [
      t
        ? t('validation.titleMaxLength', {
            max: EVENT_VALIDATION_RULES.title.maxLength,
          })
  ...
  if (!formData.startDate) {
    errors.startDate = [
      t ? t('validation.startDateRequired') : 'Start date is required',
    ];
  } else {
    ...
    if (startDate < now) {
      warnings.startDate = [
        t ? t('validation.startDatePast') : 'Start date is in the past',
      ];
    }
  }
  ...
  if (formData.capacity !== undefined) {
    if (formData.capacity < EVENT_VALIDATION_RULES.capacity.min) {
      errors.capacity = [
        t
          ? t('validation.capacityMin', {
              min: EVENT_VALIDATION_RULES.capacity.min,
            })
          : `Capacity must be at least ${EVENT_VALIDATION_RULES.capacity.min}`,
      ];
    } else if (formData.capacity > EVENT_VALIDATION_RULES.capacity.max) {
      errors.capacity = [
        t
          ? t('validation.capacityMax', {
              max: EVENT_VALIDATION_RULES.capacity.max,
            })
          : `Capacity must be no more than ${EVENT_VALIDATION_RULES.capacity.max}`,
      ];
    }
  }
  ...
  if (
    ...
    formData.description.length > EVENT_VALIDATION_RULES.description.maxLength
  ) {
    errors.description = [
      t
        ? t('validation.descriptionMaxLength', {
            max: EVENT_VALIDATION_RULES.description.maxLength,
          })
  ...
  if (
    ...
    errors.shortDescription = [
      t
        ? t('validation.shortDescriptionMaxLength', {
            max: EVENT_VALIDATION_RULES.shortDescription.maxLength,
          })
  ...
  if (
    formData.tags &&
    formData.tags.length > EVENT_VALIDATION_RULES.tags.maxItems
  ) {
    errors.tags = [
      t
        ? t('validation.tagsMaxItems', {
            max: EVENT_VALIDATION_RULES.tags.maxItems,
          })
        : `Maximum ${EVENT_VALIDATION_RULES.tags.maxItems} tags allowed`,
    ];
  }

  if (formData.tags) {
    const invalidTags = formData.tags.filter(
      tag => tag.length > EVENT_VALIDATION_RULES.tags.maxLength
    );
    if (invalidTags.length > 0) {
      errors.tags = [
        ...(errors.tags || []),
        t
          ? t('validation.tagsMaxLength', {
              max: EVENT_VALIDATION_RULES.tags.maxLength,
            })
          : `Tags must be no more than ${EVENT_VALIDATION_RULES.tags.maxLength} characters each`,
      ];
    }
  }
  ...
  if (
    formData.features &&
    formData.features.length > EVENT_VALIDATION_RULES.features.maxItems
  ) {
    errors.features = [
      t
        ? t('validation.featuresMaxItems', {
            max: EVENT_VALIDATION_RULES.features.maxItems,
          })
  ...
  if (formData.features) {
    const invalidFeatures = formData.features.filter(
      feature => feature.length > EVENT_VALIDATION_RULES.features.maxLength
    );
    if (invalidFeatures.length > 0) {
      errors.features = [
        ...(errors.features || []),
        t
          ? t('validation.featuresMaxLength', {
              max: EVENT_VALIDATION_RULES.features.maxLength,
            })
          : `Features must be no more than ${EVENT_VALIDATION_RULES.features.maxLength} characters each`,
      ];
    }
  }
  ...
  if (formData.social) {
    ...
    Object.entries(formData.social).forEach(([platform, url]) => {
      if (url && !urlPattern.test(url)) {
        socialErrors.push(
          t
            ? t('validation.socialUrlInvalid', { platform })
  ...
  if (formData.contact) {
    ...
    if (
      ...
      !/^[+]?[1-9][\d]{0,15}$/.test(
        formData.contact.phone.replace(/[\s\-()]/g, '')
      )
    ) {
      contactErrors.push(
        t ? t('validation.phoneInvalid') : 'Phone number is invalid'
      );
    }
  ...
  if (formData.schedule) {
    ...
    formData.schedule.forEach((item, index) => {
      if (!item.title) {
        scheduleErrors.push(
          t
            ? t('validation.scheduleTitleRequired', { index: index + 1 })
            : `Schedule item ${index + 1} title is required`
        );
      }
      if (!item.startsAt) {
        scheduleErrors.push(
          t
            ? t('validation.scheduleStartRequired', { index: index + 1 })
            : `Schedule item ${index + 1} start time is required`
        );
      }
      if (item.endsAt && item.startsAt) {
        ...
        if (end <= start) {
          scheduleErrors.push(
            t
              ? t('validation.scheduleEndAfterStart', { index: index + 1 })
              : `Schedule item ${index + 1} end time must be after start time`
          );
        }
      }
    });
...
export const validateEventForPublication = (
  event: Event,
  t?: (key: string) => string
): {
  isReady: boolean;
  ...
  // Required fields
  if (!event.title)
    missingFields.push(t ? t('validation.missingTitle') : 'Title');
  if (!event.startDate)
    missingFields.push(t ? t('validation.missingStartDate') : 'Start date');
  if (!event.description)
    missingFields.push(t ? t('validation.missingDescription') : 'Description');
  if (!event.venue)
    missingFields.push(t ? t('validation.missingVenue') : 'Venue');
  if (!event.category)
    missingFields.push(t ? t('validation.missingCategory') : 'Category');

  // Recommended fields
  if (!event.shortDescription)
    recommendations.push(
      t
        ? t('validation.recommendShortDescription')
        : 'Add a short description for better display'
    );
  if (!event.metadata?.heroImageUrl)
    recommendations.push(
      t
        ? t('validation.recommendHeroImage')
        : 'Add a hero image to make the event more attractive'
    );
  if (!event.metadata?.social)
    recommendations.push(
      t
        ? t('validation.recommendSocial')
        : 'Add social media links to increase visibility'
    );
  if (!event.metadata?.contact)
    recommendations.push(
      t
        ? t('validation.recommendContact')
        : 'Add contact information for attendees'
    );
  if (!event.tags || event.tags.length === 0)
    recommendations.push(
      t
        ? t('validation.recommendTags')
        : 'Add tags to help people find your event'
    );
  if (!event.capacity)
    recommendations.push(
      t
        ? t('validation.recommendCapacity')
        : 'Set a capacity limit to manage attendance'
    );
...
export const getEventQualityScore = (
  event: Event
): {
  score: number;
  maxScore: number;
  ...
  breakdown: Record<
    string,
    { score: number; maxScore: number; weight: number }
  >;
} => {
  ...
  if (event.metadata?.social?.instagram || event.metadata?.social?.facebook)
    breakdown.social.score += 1;
