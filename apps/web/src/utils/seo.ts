...
export const generateCanonicalUrl = (
  path: string,
  baseUrl: string = 'https://linea-production.up.railway.app'
): string => {
...
export const generateOwnerSEOData = (owner: {
  ...
  return {
    ...
    canonicalUrl: generateCanonicalUrl(
      `/designers/${owner.name.toLowerCase().replace(/\s+/g, '-')}`
    ),
    ogImage: owner.socialLinks?.website
      ? `${owner.socialLinks.website}/og-image.jpg`
      : undefined,
    ogType: 'profile',
...
export const generateOwnerStructuredData = (seoData: OwnerSEOData) => {
  return {
    ...
    ...(seoData.socialLinks?.instagram && {
      sameAs: [
        seoData.socialLinks.instagram,
        ...Object.values(seoData.socialLinks).filter(
          link => link && link !== seoData.socialLinks?.instagram
        ),
      ],
    }),
    ...(seoData.events &&
      seoData.events.length > 0 && {
  ...
  return {
    ...
    ...(seoData.venue && {
      location: {
        ...
        ...(seoData.venue.latitude &&
          seoData.venue.longitude && {
            geo: {
              '@type': 'GeoCoordinates',
