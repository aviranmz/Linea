...
async function populateWaitlists() {
  try {
    ...
    for (const event of finalEvents) {
      const percentage = event.capacity ? Math.round((event._count.waitlistEntries / event.capacity) * 100) : 0
      console.log(`   - ${event.title}: ${event._count.waitlistEntries}/${event.capacity} (${percentage}%)`)
    }
