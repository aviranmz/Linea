...
async function populateWaitlists() {
  try {
    ...
    console.log(
      `📅 Processing ${eventsWithCapacity.length} events with capacity...`
    );
    ...
    for (const event of eventsWithCapacity) {
      ...
      const currentWaitlist = await prisma.waitlistEntry.count({
      ...
      console.log(
        `   Target: ${targetPercentage}% (${targetCount} users, ${usersToAdd} to add)`
      );
    ...
    for (const event of finalEvents) {
      const percentage = event.capacity
        ? Math.round((event._count.waitlistEntries / event.capacity) * 100)
        : 0;
      console.log(
        `   - ${event.title}: ${event._count.waitlistEntries}/${event.capacity} (${percentage}%)`
      );
    }
  } catch (error) {
