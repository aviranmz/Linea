...
async function run() {
  ...
  for (const v of venues) {
    venueChecked++;
    const needsCityFix =
      v.city?.toLowerCase() !== 'milano' ||
      v.country?.toLowerCase() !== 'italy';
    const needsCoordFix = !isInMilanBounds(
      v.latitude ?? null,
      v.longitude ?? null
    );
  ...
  for (const e of events) {
    ...
    const wrongCity =
      e.venue &&
      (e.venue.city?.toLowerCase() !== 'milano' ||
        e.venue.country?.toLowerCase() !== 'italy');
    if (outOfBounds || wrongCity) {
      await prisma.event.update({
        ...
        data: {
          ...
          mapAddress:
            e.mapAddress && e.mapAddress.toLowerCase().includes('milano')
              ? e.mapAddress
              : 'Milano, Italy',
        },
      });
