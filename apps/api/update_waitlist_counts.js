...
async function updateWaitlistCounts() {
  try {
    ...
    for (const event of events) {
      ...
      console.log(`✅ Updated "${event.title}": ${waitlistCount} waitlist entries`)
    }
    ...
    for (const event of finalEvents) {
      const percentage = event.capacity ? Math.round((event.currentWaitlist / event.capacity) * 100) : 0
      console.log(`   - ${event.title}: ${event.currentWaitlist}/${event.capacity || 'unlimited'} (${percentage}%)`)
    }
