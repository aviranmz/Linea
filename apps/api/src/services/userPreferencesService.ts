...
export class UserPreferencesService {
  ...
  async updatePreferencePath(
    ...
    switch (operation) {
      case 'set':
        updatedPreferences = this.setNestedValue(currentPreferences, path, value)
        break
      case 'merge': {
        const existingValue = this.getNestedValue(currentPreferences, path) || {}
        const mergedValue = this.deepMerge(existingValue, value)
        updatedPreferences = this.setNestedValue(currentPreferences, path, mergedValue)
        break
      }
      case 'remove':
  ...
  async batchUpdatePreferences(
    ...
    for (const update of updates) {
      switch (update.operation) {
        case 'set':
          currentPreferences = this.setNestedValue(currentPreferences, update.path, update.value)
          break
        case 'merge': {
          const existingValue = this.getNestedValue(currentPreferences, update.path) || {}
          const mergedValue = this.deepMerge(existingValue, update.value)
          currentPreferences = this.setNestedValue(currentPreferences, update.path, mergedValue)
          break
        }
        case 'remove':
          currentPreferences = this.removeNestedValue(currentPreferences, update.path)
          break
      }
    }
  ...
  async resetUserPreferences(userId: string, category?: keyof UserPreferences): Promise<UserPreferences> {
    const currentPreferences = await this.getUserPreferences(userId)
  ...
  async exportUserPreferences(userId: string): Promise<{ preferences: UserPreferences; exportedAt: Date }> {
  ...
  async getPreferencesByCategory(userId: string, category: keyof UserPreferences): Promise<any> {
    const preferences = await this.getUserPreferences(userId)
  ...
  async validatePreferences(preferences: Partial<UserPreferences>): Promise<{ valid: boolean; errors: string[] }> {
    ...
    if (preferences.theme) {
      if (preferences.theme.primaryColor && !this.isValidColor(preferences.theme.primaryColor)) {
        errors.push('Invalid primary color format')
      }
      if (preferences.theme.secondaryColor && !this.isValidColor(preferences.theme.secondaryColor)) {
    ...
    if (preferences.privacy) {
      if (preferences.privacy.profileVisibility && !['public', 'private', 'friends'].includes(preferences.privacy.profileVisibility)) {
    ...
    if (preferences.localization) {
      ...
      if (preferences.localization.language && !validLanguages.includes(preferences.localization.language)) {
