...
import { MessageAdapter, Message, MessageResult, MessageChannel } from '../adapters/messageAdapters.js'
...
export class MessageService {
  ...
  constructor(config: MessageServiceConfig) {
    ...
    // Initialize adapters
    if (config.email) {
      this.adapters.set(MessageChannel.EMAIL, new EmailAdapter(
        config.email.apiKey,
    ...
    if (config.telegram) {
      this.adapters.set(MessageChannel.TELEGRAM, new TelegramAdapter(
    ...
    if (config.whatsapp) {
      this.adapters.set(MessageChannel.WHATSAPP, new WhatsAppAdapter(
    ...
    if (config.sms) {
      this.adapters.set(MessageChannel.SMS, new SMSAdapter(
  ...
  private async sendWithRetry(adapter: MessageAdapter, message: Message): Promise<MessageResult> {
    let lastError: string | undefined
    let retryCount = 0
    ...
    while (retryCount <= (this.retryConfig?.maxRetries || 3)) {
      ...
      if (retryCount < (this.retryConfig?.maxRetries || 3)) {
        const delay = (this.retryConfig?.retryDelay || 1000) * Math.pow(this.retryConfig?.backoffMultiplier || 2, retryCount)
  ...
  private async getAvailableAdapters(channel: MessageChannel): Promise<MessageAdapter[]> {
  ...
  private getFallbackChannels(channel: MessageChannel): MessageChannel[] {
    ...
    const fallbackMap: Record<MessageChannel, MessageChannel[]> = {
      [MessageChannel.EMAIL]: [MessageChannel.TELEGRAM, MessageChannel.WHATSAPP, MessageChannel.SMS],
      [MessageChannel.TELEGRAM]: [MessageChannel.WHATSAPP, MessageChannel.EMAIL, MessageChannel.SMS],
      [MessageChannel.WHATSAPP]: [MessageChannel.SMS, MessageChannel.TELEGRAM, MessageChannel.EMAIL],
      [MessageChannel.SMS]: [MessageChannel.WHATSAPP, MessageChannel.TELEGRAM, MessageChannel.EMAIL]
    }
  ...
  private chunkMessage(message: Message): Message[] {
    ...
    for (const part of splitPoints) {
      if (currentChunk.length + part.length + 1 <= (this.chunkingConfig?.maxLength || 1000)) {
        currentChunk += (currentChunk ? (this.chunkingConfig?.splitOn || '\n') : '') + part
      } else {
        if (currentChunk) {
          chunks.push({
            ...message,
