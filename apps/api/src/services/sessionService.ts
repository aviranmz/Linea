...
export class SessionService {
  ...
  constructor() {
    ...
    const forceMockSessions =
      String(process.env.SESSION_MOCK).toLowerCase() === 'true';

    if (forceMockSessions) {
      console.warn(
        'SESSION_MOCK=true → Using in-memory sessions (Redis disabled)'
      );
      this.client = null;
    ...
    if (
      ...
      config.redis.REDIS_URL.includes('production-redis')
    ) {
  ...
  async cleanupExpiredSessions(): Promise<number> {
    ...
    if (expiredKeys.length > 0 && this.client)
