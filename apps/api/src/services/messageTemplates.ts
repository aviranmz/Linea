...
Best regards,
The Linea Team`,
      channels: [MessageChannel.EMAIL, MessageChannel.TELEGRAM, MessageChannel.WHATSAPP],
      variables: ['eventTitle', 'eventDescription', 'eventDate', 'eventLocation', 'organizerName', 'joinLink'],
      priority: 'normal'
    })
...
The Linea Team`,
      ...
      variables: ['eventTitle', 'eventDescription', 'eventDate', 'eventLocation'],
      priority: 'normal'
    })
...
See you there!
The Linea Team`,
      channels: [MessageChannel.EMAIL, MessageChannel.TELEGRAM, MessageChannel.WHATSAPP, MessageChannel.SMS],
      variables: ['eventTitle', 'eventDate', 'eventLocation', 'eventTime', 'joinLink'],
      priority: 'high'
    })
...
The Linea Team`,
      ...
      variables: ['alertType', 'alertMessage', 'timestamp', 'severity', 'actionRequired'],
      priority: 'urgent'
    })
  }
  ...
  renderTemplate(templateId: string, variables: TemplateVariables, channel: MessageChannel): Message | null {
    ...
    if (!template.channels.includes(channel)) {
      throw new Error(`Template ${templateId} not supported for channel ${channel}`)
    }
    ...
    const subject = template.subject ? this.renderContent(template.subject, variables, channel) : undefined
  ...
  private renderContent(content: string, variables: TemplateVariables, channel: MessageChannel): string {
  ...
  getTemplatesByChannel(channel: MessageChannel): MessageTemplate[] {
    return this.getAllTemplates().filter(template => template.channels.includes(channel))
  }
}
