...
import { WebhookService, TelegramWebhookHandler, WhatsAppWebhookHandler, SMSWebhookHandler, EmailWebhookHandler } from '../services/webhookService.js'
...
export async function webhookRoutes(fastify: FastifyInstance) {
  ...
  const telegramHandler = new TelegramWebhookHandler(process.env.TELEGRAM_BOT_TOKEN || '')
  const whatsappHandler = new WhatsAppWebhookHandler(process.env.WHATSAPP_WEBHOOK_SECRET || '')
  const smsHandler = new SMSWebhookHandler(process.env.TWILIO_WEBHOOK_SECRET || '')
  const emailHandler = new EmailWebhookHandler(process.env.SENDGRID_WEBHOOK_SECRET || '')
  ...
  fastify.get('/webhooks/messages', {
    ...
    try {
      const { platform, limit } = request.query as { platform?: string; limit?: number }
      const messages = await webhookService.getMessageHistory(platform, limit)
      return { messages }
    } catch (error) {
      fastify.log.error(error)
      return reply.status(500).send({ error: 'Internal server error' })
  ...
  fastify.get('/webhooks/telegram/verify', {
    schema: {
  ...
  fastify.get('/webhooks/whatsapp/verify', {
    schema: {
