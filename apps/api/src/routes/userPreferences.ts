...
export async function userPreferencesRoutes(fastify: FastifyInstance) {
  ...
  fastify.patch('/preferences/:path*', {
    schema: {
    ...
    try {
      ...
      const { value, operation = 'set' } = request.body as { value: any; operation?: 'set' | 'merge' | 'remove' }

      const updatedPreferences = await preferencesService.updatePreferencePath(userId, path, value, operation)
      return { preferences: updatedPreferences }
    } catch (error) {
      fastify.log.error(error)
      return reply.status(500).send({ error: 'Internal server error' })
  ...
  fastify.patch('/preferences/batch', {
    schema: {
    ...
    try {
      ...
      const updatedPreferences = await preferencesService.batchUpdatePreferences(userId, updates)
      return { preferences: updatedPreferences }
    } catch (error) {
      fastify.log.error(error)
      return reply.status(500).send({ error: 'Internal server error' })
  ...
  fastify.delete('/preferences', {
    ...
    try {
      ...
      const updatedPreferences = await preferencesService.resetUserPreferences(userId, category as any)
      return { preferences: updatedPreferences }
    } catch (error) {
      fastify.log.error(error)
      return reply.status(500).send({ error: 'Internal server error' })
  ...
  fastify.post('/preferences/import', {
    ...
    try {
      ...
      const { preferences, merge = true } = request.body as { preferences: any; merge?: boolean }

      const updatedPreferences = await preferencesService.importUserPreferences(userId, preferences, merge)
      return { preferences: updatedPreferences }
    } catch (error) {
      fastify.log.error(error)
      return reply.status(500).send({ error: 'Internal server error' })
  ...
  fastify.get('/preferences/:category', {
    schema: {
    ...
    try {
      ...
      const preferences = await preferencesService.getPreferencesByCategory(userId, category as any)
      return { preferences }
    } catch (error) {
      fastify.log.error(error)
      return reply.status(500).send({ error: 'Internal server error' })
