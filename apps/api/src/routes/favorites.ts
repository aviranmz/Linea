...
export async function favoritesRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/favorites', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const user = await requireAuth(request, reply)
      if (!user) return
  ...
  fastify.delete('/api/favorites/:eventId', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const user = await requireAuth(request, reply)
      if (!user) return
  ...
  fastify.get('/api/favorites/:eventId', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const user = await requireAuth(request, reply)
      if (!user) return
  ...
  fastify.post('/api/favorites/:eventId/toggle', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const user = await requireAuth(request, reply)
      if (!user) return
